  ...
  return (
    <motion.div
      ...
      className={`rounded-md border p-2 mb-2 ${
        isCT ? 'border-[#6b8fc2]/10 bg-[#6b8fc2]/[0.03]' : 'border-[#c49a6c]/10 bg-[#c49a6c]/[0.03]'
      }`}
    >
      <div className="flex items-center justify-center gap-2 mb-1.5">
        <p className="text-gray-600 font-rajdhani text-[10px] uppercase tracking-wider text-center">
          Team Record
        </p>
      ...
      <div className="flex flex-wrap gap-1 justify-center">
        {teamMapStats.map((stat) => (
          <div
            key={stat.map}
            className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-white/[0.03] text-[10px]"
          >
            <span className="text-gray-500 font-rajdhani">{stat.map}</span>
