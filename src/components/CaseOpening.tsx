...
const VISIBLE_ITEMS = 40; // total items in the strip (repeating players)
const WINNER_INDEX = 30; // which index the winner will land on
...
export default function CaseOpening() {
  ...
  const handleOpen = useCallback(() => {
    ...
    // Target offset: center the winner item in the viewport
    // The center of the container is at containerWidth/2
    // The winner item starts at WINNER_INDEX * ITEM_TOTAL
    // We want: WINNER_INDEX * ITEM_TOTAL + ITEM_WIDTH/2 = containerWidth/2 + finalOffset
    // So finalOffset = WINNER_INDEX * ITEM_TOTAL + ITEM_WIDTH/2 - containerWidth/2
    const containerWidth = containerRef.current?.offsetWidth || 600;
    const targetOffset = WINNER_INDEX * ITEM_TOTAL + ITEM_WIDTH / 2 - containerWidth / 2;
    // Add slight randomness within the item width for realism
    ...
    const animate = (now: number) => {
      ...
      // Cubic ease-out for authentic CS2 case deceleration
      const eased = 1 - Math.pow(1 - progress, 3);
      const currentOffset = eased * finalTarget;
      ...
      if (progress < 1) {
        ...
        setTimeout(() => {
          dispatch({ type: 'SPIN_COMPLETE', payload: { playerId: winner.id } });
          setWinnerRevealed(false);
    ...
    animRef.current = requestAnimationFrame(animate);
  }, [isRolling, state.availablePlayers, dispatch, playTick, playWinner]);

  // Click on a name in the strip to pick directly
  ...
  // Build initial strip for display when not rolling
  ...
  return (
    ...
    >
      ...
      {/* Desktop: CT left | Case center | T right */}
      {/* Mobile: Case, then CT, then T */}
                  ...
                  return (
                    <div
                      ...
                      className={`
                        ...
                        font-rajdhani font-bold text-sm transition-all duration-300
                        ${isWinner
                          ? 'bg-amber-500/30 border-2 border-amber-400 text-amber-200 shadow-[0_0_20px_rgba(255,179,0,0.4)] scale-105'
                    ...
                    >
                      <span className="truncate px-2">{item.player.name}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {winnerRevealed && currentWinner && (
              ...
              >
                <span className={`font-orbitron text-lg font-bold ${
                  state.currentTurn === 'CT' ? 'text-sky-400' : 'text-amber-400'
                }`}>
                  {currentWinner.name}
                </span>
                <span className="text-gray-400 font-rajdhani text-sm ml-2">
                  joins {state.currentTurn}!
                </span>
              </motion.div>
            )}
          </AnimatePresence>
